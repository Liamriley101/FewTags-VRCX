...
namespace FewTags
{
    internal class ColorConsole
    {
        ...
        public static void Print(string Tag)
        {
            ...
            {
                if (Match.Value.StartsWith("<color=") == true)
                {
                    string HexColor = Match.Value.Replace("<color=", "").Replace(">", "");
                    HexColors.Add(HexColor);
                }
                else if (Match.Value.StartsWith("<color=") == false)
                {
                    Console.ResetColor();
                }
            }
        ...
        */
        ...
        public static void Print(string Tag)
        {
            ...
            var Matches = Regex.Matches(Tag, @"<color=#[0-9A-Fa-f]{6}>|</color>");

            foreach (Match Match in Matches)
            {
                int Index = Match.Index;
                string Text = Tag.Substring(LastIndex, Index - LastIndex);
                Output += Text;

                if (Match.Value.StartsWith("<color=") == true)
                {
                    string Hex = Match.Value.Substring(7, 7);
                    HTCC.SetConsoleColor(Hex);
                    ColorStack.Push(Hex);
                }
                else if (Match.Value.StartsWith("<color=") == false)
                {
                    HTCC.DefaultConsoleColor();
            ...
            {
                Output += Tag.Substring(LastIndex);
            }
